...
from keras.layers import Convolution2D, Dropout, Flatten, Dense, MaxPooling2D
...
class SD_NN:
    def __init__(self, testset, testdir, seed, training_perc, epochs, dropout, buckets):
    ...
    """
    #use this to test scikit neuaral networl, parse data for mnist style image data
    def parse_images_2d(self, dataset):
        #converts image stack to 2D dataframe
        def image_to_dataframe(img):
            processed_img = []
            for y in range(len(img)):
                for x in range(len(img[y])):
                    rgb = int(65536 * img[y][x][0] + 256 * img[y][x][1] + img[y][x][2])
        ...
        for point in dataset:
            img = cv2.imread(point[0])
            image_dataframe = image_to_dataframe(self.img_preprocess(img))
            #processed_dataset.append(self.img_preprocess(img))
            #processed_dataset.append(img)
            processed_dataset.append(image_dataframe)
    ...
    def train_model(self):
        ...
        model.fit(np.asarray(self.train[0]), np.asarray(self.train[1]), epochs=self.epochs)
        ...
        for i in range(len(predictions)):
            ...
            if abs(pred - actual) < .1:
                accuracy += 1
        #self.confusion_matrix(predictions, self.test[1])
    ...
    def confusion_matrix(self, predictions, labels):
        def determine_label(l):
            ...
            if l > 0:
                return "right"
            return "straight"
        label_list = ["left", "right", "straight"] # list of all possible labels
        results = [] # list of tuples formatted: (predicted label, actual label)
        for i in range(len(predictions)):
            pred = predictions[i].astype(np.float)
            actual = self.test[1][i].astype(np.float)
            results.append((pred[0], actual))
        # count up the results for each label
        matrix = {}
        ...
        for result in results:
            predicted = determine_label(float(result[0]))
            actual = determine_label(float(result[1]))
            matrix[(predicted, actual)] += 1

        # create file
        file = open("confusion_matric.csv"+str(self.epochs), "w")
        keys = list(matrix.keys())
        names = set()
        # write top row of lables
        for ele in range(len(keys)):
            names.add(keys[ele][0])
        for ele in names:
            print(str(ele)+", ", end="")
            file.write(str(ele)+", ")
        print()
        file.write("\n")
        # write inside of confusion matrix
        count = 0
        for ele1 in names:
            for ele2 in names:
                print(str(matrix[(ele2, ele1)]) + ",", end=" ")
                file.write(str(matrix[(ele2, ele1)]) + ", ")
            print(ele1)
            if count == len(names):
                file.write(str(ele1))
            else:
...
if __name__ == "__main__":
    ...
    model = SD_NN(testset, dir, seed, training_perc, epochs, dropout, buckets)
    model.train_model()
    model.save_model()
    #model.simple_keras_boi()
